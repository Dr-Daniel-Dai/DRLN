...
pretrained_model = {'densenet121' : './models/pretrained/densenet121-a639ec97.pth',}
# pretrained_model = {'vgg16' : './models/pretrained/vgg16-397923af.pth',}
...
def load_data_transformers(resize_reso=512, crop_reso=448, swap_num=[7, 7]):
    ...
    data_transforms = {
        ...
        'test_totensor': transforms.Compose([
            transforms.Resize((resize_reso, resize_reso)),
            # transforms.CenterCrop((crop_reso, crop_reso)),   ###  dy modify
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
        ]),
        'None': None,
...
class LoadConfig(object):
    def __init__(self, args, version):
        ...
        elif args.dataset == 'PID':
            self.dataset = args.dataset
            self.rawdata_root = './datasets/PID/data'
            self.anno_root = './datasets/PID/anno'
            self.numcls = 12
        ...
        if 'train' in get_list:
             self.train_anno = pd.read_csv(os.path.join(self.anno_root, 'train.txt'),\
                                           sep="/",\
                                           header=None,\
                                           names=['ImageName', 'label', 'multilabel'])
