# coding=utf8


from __future__ import division
import os
import torch
import torch.utils.data as data
import pandas
import random
import PIL.Image as Image
from PIL import ImageStat
import numpy as np
import pdb


def random_sample(img_names, labels):
    anno_dict = {}
    img_list = []
    anno_list = []
    for img, anno in zip(img_names, labels):
        if not anno in anno_dict:
            anno_dict[anno] = [img]
        else:
            anno_dict[anno].append(img)

    for anno in anno_dict.keys():
        anno_len = len(anno_dict[anno])
        fetch_keys = random.sample(list(range(anno_len)), anno_len//10)
        img_list.extend([anno_dict[anno][x] for x in fetch_keys])
        anno_list.extend([anno for x in fetch_keys])
    return img_list, anno_list


class dataset(data.Dataset):
    def __init__(self, Config, anno, swap_size=[7,7], common_aug=None, swap=None, totensor=None, train=False, train_val=False, test=False):
        self.root_path = Config.rawdata_root
        self.numcls = Config.numcls
        self.dataset = Config.dataset
        self.use_cls_2 = Config.cls_2
        self.use_cls_mul = Config.cls_2xmul
        self.anno_dy = anno
        
        self.train = train
        
        self.trX = anno['ImageName'].tolist()
        self.trY = np.load('./datasets/PID/anno/trY_transfer_2.npy')
        
        shuffling = list(zip(self.trX, self.trY))
        random.shuffle(shuffling)
        random.shuffle(shuffling)
        self.trX, self.trY = zip(*shuffling) 

        self.trY = np.asarray(self.trY)
        self.trX_dy = []
        self.teX_dy = []
        
        shutpoint = 2800     #### 12600   11200
        for i in range(shutpoint):
            self.trX_dy.append(str (self.trX[i]) )
            # print(self.trX_dy)
        for j in range(shutpoint,14000):
            self.teX_dy.append(str(self.trX[j]) )
            
            
        self.teY =  self.trY [ shutpoint:  ]  # testset
        self.trY =  self.trY [ :shutpoint  ]  # trainset
        
        if isinstance(anno, pandas.core.frame.DataFrame):
            # self.paths = anno['ImageName'].tolist()
            # self.labels = np.load('./datasets/PID/anno/PIDnumpy_train.npy')
            
            
            # trX = anno['ImageName'].tolist()
            # trY = np.load('./datasets/PID/anno/PIDnumpy_train.npy')
            
            # shuffling = list(zip(trX, trY))
            # random.shuffle(shuffling)
            # random.shuffle(shuffling)
            # trX, trY = zip(*shuffling) 
    
            # trY = np.asarray(trY)
            # trX_dy = []
            # teX_dy = []
            
            # shutpoint = 10000     #### 12600   11200
            # for i in range(shutpoint):
            #     trX_dy.extend(trX[i] )
            # for j in range(shutpoint,11200):
            #     teX_dy.extend(trX[j] )
                
            # teY =  trY [ shutpoint:  ]  # testset
            # trY =  trY [ :shutpoint  ]  # trainset
            
            if self.train:
                self.paths = self.trX_dy
                self.labels = self.trY
            else:
                self.paths = self.teX_dy
                self.labels = self.teY
                
            # if self.train:
            #     self.labels = np.load('./datasets/PID/anno/PIDnumpy_train.npy')
            # else:
            #     self.labels = np.load('./datasets/PID/anno/PIDnumpy_test.npy')
            
#####   dy modify
        # if train_val:
        #     self.paths, self.labels = random_sample(self.paths, self.labels)
        
        self.common_aug = common_aug
        self.swap = swap
        self.totensor = totensor
        self.cfg = Config
        
        self.swap_size = swap_size
        self.test = test

    def __len__(self):
        return len(self.paths)

    def __getitem__(self, item):
        img_path = os.path.join(self.root_path, self.paths[item])

        img_path.replace('\\','/')
        # print (img_path)
        img = self.pil_loader(img_path)

        # if self.test:
        #     img = self.totensor(img)
        #     label = self.labels[item]
            
        #     return img, label, self.paths[item]
        
        img_unswap = self.common_aug(img) if not self.common_aug is None else img

        image_unswap_list = self.crop_image(img_unswap, self.swap_size)

        swap_range = self.swap_size[0] * self.swap_size[1]
        swap_law1 = [(i-(swap_range//2))/swap_range for i in range(swap_range)]    ###  '//'	取整除（向下取整）
        swap_law1_index = [i for i in range(swap_range)]    ###  '//'	取整除（向下取整）

        if self.train:
            img_swap, temx, temy = self.swap(img_unswap)
            image_swap_list = self.crop_image(img_swap, self.swap_size)
            
            unswap_stats = [sum(ImageStat.Stat(im).mean) for im in image_unswap_list]
            swap_stats = [sum(ImageStat.Stat(im).mean) for im in image_swap_list]
            
            swap_law2 = []
            swap_law2_index= []
            swap_dy_index = []
            
            for idy in range(len(temy)):
                for idx in range(len(temy)):
                    
                    index_ = temy[idy]*7 + temx[temy[idy]*7+idx]
                    
                    swap_dy_index.append(index_)
            
            for h in range(49):
                index = swap_dy_index.index(h)
                swap_law2_index.append(index)
                        
            for swap_im in swap_stats:
                distance = [abs(swap_im - unswap_im) for unswap_im in unswap_stats]
                index = distance.index(min(distance))
                swap_law2.append((index-(swap_range//2))/swap_range)
                
            img_swap = self.totensor(img_swap)
            
            label = self.labels[item]
            
            if self.use_cls_mul:
                label_swap = label + self.numcls
            if self.use_cls_2:
                label_swap = -1
            img_unswap = self.totensor(img_unswap)
            return img_unswap, img_swap, label, label_swap, swap_law1, swap_law2, swap_law1_index, swap_law2_index, self.paths[item]
        else:
            # if isinstance(self.anno_dy, pandas.core.frame.DataFrame):
            #     self.labels_single = self.anno_dy['label'].tolist()
            # label_swap = self.labels_single[item]
            
            # label = self.labels[item]
            
            # swap_law2 = [(i-(swap_range//2))/swap_range for i in range(swap_range)]
            # # label_swap = label
            # img_unswap = self.totensor(img_unswap)
            # return img_unswap, label, label_swap, swap_law1, swap_law2, self.paths[item]
            ####################################################################
            img_swap, temx, temy = self.swap(img_unswap)
            image_swap_list = self.crop_image(img_swap, self.swap_size)
            
            unswap_stats = [sum(ImageStat.Stat(im).mean) for im in image_unswap_list]
            swap_stats = [sum(ImageStat.Stat(im).mean) for im in image_swap_list]
            
            swap_law2 = []
            swap_law2_index= []
            swap_dy_index = []
            
            for idy in range(len(temy)):
                for idx in range(len(temy)):
                    
                    index_ = temy[idy]*7 + temx[temy[idy]*7+idx]
                    
                    swap_dy_index.append(index_)
            
            for h in range(49):
                index = swap_dy_index.index(h)
                swap_law2_index.append(index)
                    
            for swap_im in swap_stats:
                distance = [abs(swap_im - unswap_im) for unswap_im in unswap_stats]
                index = distance.index(min(distance))
                swap_law2.append((index-(swap_range//2))/swap_range)
                
                
            img_swap = self.totensor(img_swap)
            
            label = self.labels[item]
            
            if self.use_cls_mul:
                label_swap = label + self.numcls
            if self.use_cls_2:
                label_swap = -1
            img_unswap = self.totensor(img_unswap)
            return img_unswap, img_swap, label, label_swap, swap_law1, swap_law2, swap_law1_index, swap_law2_index, self.paths[item]

    def pil_loader(self,imgpath):
        with open(imgpath, 'rb') as f:
            with Image.open(f) as img:
                return img.convert('RGB')

    def crop_image(self, image, cropnum):
        width, high = image.size
        crop_x = [int((width / cropnum[0]) * i) for i in range(cropnum[0] + 1)]
        crop_y = [int((high / cropnum[1]) * i) for i in range(cropnum[1] + 1)]
        im_list = []
        for j in range(len(crop_y) - 1):
            for i in range(len(crop_x) - 1):
                im_list.append(image.crop((crop_x[i], crop_y[j], min(crop_x[i + 1], width), min(crop_y[j + 1], high))))
        return im_list


    def get_weighted_sampler(self):
        img_nums = len(self.labels)
        weights = [self.labels.count(x) for x in range(self.numcls)]
        return torch.utils.data.sampler.WeightedRandomSampler(weights, num_samples=img_nums)


def collate_fn4train(batch):
    imgs = []
    label = []
    label_swap = []
    law_swap = []
    law_index = []
    img_name = []
    
    for sample in batch:
        imgs.append(sample[0])    ##### img_unswap, 
        imgs.append(sample[1])       ###### img_swap, 
        label.append(sample[2])      ######  label, 
        label.append(sample[2])      ######  label, 
        
        if sample[3] == -1:
            label_swap.append(1)     
            label_swap.append(0)
        else:
            label_swap.append(sample[2])    ###### label,
            label_swap.append(sample[3])    ###### label_swap, 
            
        law_swap.append(sample[4])   ##### swap_law1, 
        law_swap.append(sample[5])    #####  swap_law2, 
        law_index.append(sample[6])
        law_index.append(sample[7])
        
        img_name.append(sample[-1])     #####  self.paths[item]
    return torch.stack(imgs, 0), label, label_swap, law_swap, law_index, img_name

def collate_fn4val(batch):
    imgs = []
    label = []
    label_swap = []
    law_swap = []
    law_index = []
    img_name = []
    
    for sample in batch:
        imgs.append(sample[0])    ##### img_unswap, 
        imgs.append(sample[1])       ###### img_swap, 
        label.append(sample[2])      ######  label, 
        label.append(sample[2])      ######  label, 
        
        if sample[3] == -1:
            label_swap.append(1)     
            label_swap.append(0)
        else:
            label_swap.append(sample[2])    ###### label,
            label_swap.append(sample[3])    ###### label_swap, 
            
        law_swap.append(sample[4])   ##### swap_law1, 
        law_swap.append(sample[5])    #####  swap_law2, 
        law_index.append(sample[6])
        law_index.append(sample[7])
        
        img_name.append(sample[-1])     #####  self.paths[item]
    return torch.stack(imgs, 0), label, label_swap, law_swap, law_index, img_name

# def collate_fn4val(batch):
#     imgs = []
#     label = []
#     label_swap = []
#     law_swap = []
#     img_name = []
#     for sample in batch:
#         imgs.append(sample[0])     #####   img_unswap, 
#         label.append(sample[1])    #####   label, 
#         if sample[3] == -1:
#             label_swap.append(1)
#         else:
#             label_swap.append(sample[2])    #####  label_swap, 
#         law_swap.append(sample[3])      #####  swap_law1, swap_law2, 
#         img_name.append(sample[-1])      #####  self.paths[item]
#     return torch.stack(imgs, 0), label, label_swap, law_swap, img_name

def collate_fn4backbone(batch):
    imgs = []
    label = []
    img_name = []
    for sample in batch:
        imgs.append(sample[0])
        if len(sample) == 7:
            label.append(sample[2])
        else:
            label.append(sample[1])
        img_name.append(sample[-1])
    return torch.stack(imgs, 0), label, img_name


def collate_fn4test(batch):
    imgs = []
    label = []
    img_name = []
    for sample in batch:
        imgs.append(sample[0])
        label.append(sample[1])
        img_name.append(sample[-1])
    return torch.stack(imgs, 0), label, img_name
